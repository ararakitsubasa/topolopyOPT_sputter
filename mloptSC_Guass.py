...
from ba_lowmem_Guass import ba
...
class Mlopt():
    def __init__(self, x, y, node=512, n_block=1, device='cuda', truss=None):
        self.x = x
        self.y = y
    ...
    # train the DNN
    def train(self, n_epochs=1000, sample_interval=200, lr=0.01, batch_size=1024, n_train=None, decay=1., log_name=None, log_root=None, epc=0):
        x = self.x[:n_train, :]
        y = self.y[:n_train, :]
        ...
        for epoch in range(n_epochs):
            ...
            if sample_interval > 0 and epoch % sample_interval == 0:
                # torch.save(loss, os.path.join('./log', f"{'result_new.log'.split('.')[0]}_loss.pt"))
        ...
        if sample_interval >= 0:
            torch.save(torch.tensor(loss_list), os.path.join(f"{log_root}/loss", f"{log_name.split('.')[0]}_loss_{epc}.pt"))
            print('epoch', epoch, 'train_loss', loss.data.cpu().numpy())
            time_end = time.time()
    ...
    # apply BA to the DNN
    def opt(self, n=500000, maxiter=2000, seed=None, topk=10, is_min=False, print_time=True, **kwargs):
        ...
        # print(v)
        # print('------------v-------------')
        # print(1 / (v * self.std_of_y + self.mean_of_y).item())
        # print('------------predic-------------')
