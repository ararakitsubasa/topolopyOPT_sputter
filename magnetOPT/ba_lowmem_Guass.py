...
def ba(func, dim=100, n=200, maxiter=2000, is_min=True, topk=100, topk_coef=10, fmin=0, fmax=1, loud_A=0.95, r0=0.85, alpha=0.999, gamma=0.1,
    ...
    for t in m:
        ...
        for i in range(n_batch):
            ...
            x_new = torch.where(accept, best_s.expand(n_per_batch, dim) + loud_A * torch.rand(n_per_batch, dim, device=device, dtype=dtype), x_new)

            # adjust bound
            x_new.masked_fill_(x_new > 1., 1.)
            x_new.masked_fill_(x_new < 0., 0.)
            # x_new = torch.div(x_new.T, torch.sum(x_new, 1))
            # x_new = x_new.T
            # print(x_new)
